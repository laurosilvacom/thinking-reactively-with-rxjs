...
[02:54] I'm also going to create an _Observable_ factory called `keyPressed`, which is going to take in a `key` as the input and it's going to return an _Observable_ that emits anytime that specific key is pressed. Cool. Now that we have our building blocks, we can start assembling them to solve our problem.
...
[03:33] The `comboInitiator` will be the first letter from our combo. Whenever the `comboInitiator` is pressed, we want to `switchMap()` to another _Observable_ that only becomes alive when the combo has started.
...
[04:57] Finally, we want to _takeUntil_ we get _comboLength - 1_ keys back. `take(keyCombo.length - 1)`. _take()_ keeps its source alive until it receives _keyCombo.length - 1_ emissions. After that, it just terminates the source.
...
[06:33] Basically, we want to skip the first _combo length - 2_ values inside the inner combo and only take the last one. In that case, I'll add the `skip(keyCombo.length - 2)` right before the `take()` and then I'll just take the last `1`.
